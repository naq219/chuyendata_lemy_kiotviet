...
app.get('/api/orders', async (req: Request, res: Response) => {
  try {
    ...
    if (orderIds) {
      // Nếu có tham số ids, tìm kiếm theo order IDs cụ thể
      const idsArray = orderIds.split(',').map(id => id.trim()).filter(id => id.length > 0);
      if (idsArray.length > 0) {
        whereCondition = `AND o.id IN (${idsArray.join(',')})`;
      }
    } else {
      // Nếu không có tham số ids, tìm kiếm theo điều kiện cũ: WHERE p.name LIKE '%nck1%'
    ...
    if (!mapping.customers[customerId]) {
      ...
      try {
        ...
        if ('data' in createdCustomer && createdCustomer.data && typeof createdCustomer.data === 'object' && 'id' in createdCustomer.data) {
          ...
          return res.status(500).json({ success: false, error: 'Cannot find customer ID in response: ' + JSON.stringify(createdCustomer) });
        }
        ...
        if (error.errorMessage?.includes('Mã khách hàng') && error.errorMessage?.includes('đã tồn tại')) {
          ...
          if (existingCustomers?.data && existingCustomers.data.length > 0) {
            ...
            console.error('❌ Customer tồn tại nhưng không có ID');
            return res.status(500).json({ success: false, error: 'Customer tồn tại nhưng không có ID' });
          }
          } else {
            console.error('❌ Không tìm thấy customer đã tồn tại với code:', customerCode);
            return res.status(500).json({ success: false, error: `Không tìm thấy customer với code: ${customerCode}` });
          }
        } else {
        ...
        try {
          ...
          if (productError.errorMessage?.includes('đã tồn tại')) {
            ...
            if (existingProducts?.data && existingProducts.data.length > 0) {
              ...
              return res.status(500).json({ success: false, error: `Không tìm thấy product với code: ${code}` });
            }
          } else {
            // Nếu là lỗi khác, throw lại
