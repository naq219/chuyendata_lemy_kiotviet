  ...
  router.post('/api/validate-mapping', async (req: Request, res: Response) => {  // ⚠️ removed `/api` prefix
    try {
      ...
      const result: ValidationResult = {
        total: {
          customers: Object.keys(oldMapping.customers || {}).length,
          products: Object.keys(oldMapping.products || {}).length,
          orders: Object.keys(oldMapping.createdOrders || {}).length,
        },
      ...
      for (const [lemydeId, kiotvietId] of Object.entries(oldMapping.customers || {})) {
        try {
      ...
      for (const [lemydeId, kiotvietId] of Object.entries(oldMapping.products || {})) {
        try {
      ...
      for (const [lemydeOrderId, orderData] of Object.entries(oldMapping.createdOrders || {})) {
        try {
